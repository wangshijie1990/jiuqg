...
Page({
      ...
      getGoodsDetail() {
            ...
            App._get('goods/detail', {
                  ...
                  _this.setData(data);
                  //如果商品正在进行秒杀，刚显示倒计时
                  //console.log(data.detail.quick_status);
                  if(data.detail.quick_status=='no_begin'){
                        let temp = _this.data.quick_time;
                        temp.push({ date: data.detail.quick_time });
                        _this.setData({
                              'quick_time': temp,
                              can_buy:false
                        });
                        countDown.onSetTimeList(_this, 'quick_time');
      ...
      onShareAppMessage() {
            let _this = this;
            // 构建页面参数
            let shop_id=wx.getStorageSync('selectedShopId');
            let params = App.getShareUrlParams({
                  'goods_id': _this.data.goods_id,
                  'shopid':(shop_id?shop_id:0),
            });
            return {
       ...
       */
      onToggleTrade(e) {
            if(!this.data.can_buy){
                  App.showError('秒杀还没开始不能购买哦');
                  return false;
            }
            let _this = this;
            if (typeof e === 'object') {
